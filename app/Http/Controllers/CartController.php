...
class CartController extends Controller
{
    ...
    public function addToCartProduct(Request $request)
    {
        try {
            ...
            if($product == null) {
                $product = SellerListing::where('uuid', $request->uuid)->first();
            }
            ...
            if ($product == null) {
                ...
                $productName = $product->product_name ? $product->product_name : $product->name;
                ...
                if (Cart::count() > 0) {
                    ...
                    if(count($res) == 0) {
                        ...
                        $status = true;
                        $message = $productName.' added in cart.';
                        Cart::add(['id' => $product->id, 'name' => $productName, 'qty' => $quantity, 'price' => $product->price, 'weight' => 0, 'options' => ['productImage' => (! empty($product->images) ? $product->images->first()->path : null)]]);
                    } else {
                        // $status = false;
        ...
        try {
            ...
            if ($product == null) {
                ...
                if (Cart::count() > 0) {
                    ...
                    $cartItem = Cart::add(['id' => $product->id, 'name' => $productName, 'qty' => 1, 'price' => $product->price, 'weight' => 0, 'options' => ['productImage' => (! empty($product->images) ? $product->images->first()->path : null)]]);
                } else {
                    $status = true;
                    $message = $productName.' added in cart.';
                    $cartItem = Cart::add(['id' => $product->id, 'name' => $productName, 'qty' => 1, 'price' => $product->price, 'weight' => 0, 'options' => ['productImage' => (! empty($product->images) ? $product->images->first()->path : null)]]);
